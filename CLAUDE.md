...
## Project
A guided bike search and comparator. Static HTML/CSS/JS site backed by an external SaaS bike catalog API. See `specs/PRD.md` for goals and `specs/architecture.md` for system design.

## Stack
| Layer | Technology |
|-------|-----------|
| Frontend | Vanilla HTML5 / CSS3 / JavaScript (ES modules, no framework) |
| Data | External SaaS bike catalog API (REST/JSON, read-only) |
| Hosting | Static file hosting (Netlify / GitHub Pages) |
...
# Lint (if configured)
# TODO: add eslint / stylelint commands once tooling is set up
```
...
- All API calls go through `js/api.js` — never call the SaaS API directly from page scripts
...
- Do not hardcode bike data — always fetch from the API
